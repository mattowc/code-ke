...
class WC_Cart {
	...
	/*-----------------------------------------------------------------------------------*/ 
		 ...
		 * @return string
		 */
		function get_item_data( $cart_item, $cart_item_key,$flat = false ) {
			global $woocommerce;
			...
			if ( $other_data && is_array( $other_data ) && sizeof( $other_data ) > 0 ) {
				...
				foreach ($other_data as $data ) {
					...
					if ($flat) {
						...
						$data_list[] = '<dt>'.$data['name'].':</dt><dd>'.$display_value.' - <a href="' . $_SERVER['REQUEST_URI'] . '&remove_this_option=' . $display_value . '&the_item='.$cart_item_key.'&total='.$cart_item[line_total].'">x</a></dd>';
					}
		...
		function remove_option( $remove_item, $cart_id,$total ) {
		
			global $woocommerce;
			$cart_page_id = woocommerce_get_page_id('cart');
			$cart_items = $this->cart_contents[$cart_id];
				 if($cart_items['line_total'] == $total){	
					  $index = 0;
				 foreach ($cart_items[addons] as $dd) {	
				   if ($dd['value'] == $remove_item ){
						   $cart_items['line_total'] =	$cart_items['line_total'] - ($dd['price']);
						   $cart_items['line_subtotal'] =	$cart_items['line_subtotal'] - $dd['price'];
						   array_splice($cart_items[addons],$index,1);
						 
				   }
				  $index = $index + 1;
			  }
			 
			$this->cart_contents[$cart_id] = $cart_items;
